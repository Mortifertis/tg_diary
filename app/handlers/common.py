...
async def _handle_menu_interrupt(message: Message, state: FSMContext) -> bool:
    ...
    handlers = (
        (
            "menu_create",
            lambda: entries_create.create_entry_from_menu(message, state),
        ),
        ("menu_settings", lambda: reminder_settings_menu(message)),
        ("menu_view", lambda: entries_view.view_entries(message)),
        (
            "view_find_by_id",
            lambda: entries_view.prompt_find_entry_by_id(message, state),
        ),
        (
            "view_export",
            lambda: export_handlers.export_menu_with_state(message, state),
        ),
        ("view_import", lambda: backup_handlers.import_menu(message, state)),
        ("view_backup", lambda: backup_handlers.send_backup(message)),
        ("menu_manage", lambda: entries_manage.manage_entries(message, state)),
